...
data = pandas.read_csv("pran.csv",encoding='windows-1252')
...
for i,x in data.iterrows():
    ...
    time.sleep(1)
    table_row_data=f'<tr style="text-align: center;"><td style="width: 8%; text-align: center; border: 0.5px solid #000000;">1</td><td style="width: 20%; text-align: center; border: 0.5px solid #000000;"><p><strong>Type: </strong>{"Digital weight scale"}</p><p><strong>Brand:</strong> {"-" if x.brand=="none" else x.brand}</p></td><td style="width: 10%; text-align: center; border: 0.5px solid #000000;">{capacity}</td><td style="width: 10%; text-align: center; border: 0.5px solid #000000;">{quantity}</td><td style="width: 15%; text-align: center; border: 0.5px solid #000000;">{"-" if x.denomination=="none" else x.denomination}</td><td style="width: 10%; text-align: center; border: 0.5px solid #000000;">{"-" if x.weight_class=="none" else x.weight_class}</td><td style="width: 20%; text-align: center; border: 0.5px solid #000000;">{x.fee}</td></tr>'
    ...
    time.sleep(1)
    driver.find_element(By.ID,"verification_date").send_keys("01-03-2023")
    ...
    time.sleep(1)
    driver.find_element(By.ID,"receipt_no").send_keys("351068")
    time.sleep(1)
    ...
    time.sleep(2)
    driver.find_element(By.CSS_SELECTOR,"#right_side_btn > button.btn.btn-primary").click()
