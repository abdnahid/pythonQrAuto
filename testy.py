...
data = pandas.read_csv("mep.csv", encoding='windows-1252')
word_engine = inflect.engine()
...
def insert_company(x: Series) -> dict:
    return {
        ...
        "district": x.district,
        "scales": [
            {"capacity": x.capacity, "type": x.type,"scale_type":x.scale_type, "quantity": x.quantity,
             "model": x.model if x.model != "none" else "", "brand": x.brand if x.brand != "none" else "",
             "serial": x.serial if x.serial != "none" else ""},
        ],
    ...
    if len(insertedData) == 0:
        ...
        for index, value in enumerate(insertedData):
            if value["name"] == x["name"] and value["address"] == x["address"]:
                matched_index = index
                insertedData[index]["scales"].append({"capacity": x.capacity, "type": x.type, "scale_type":x.scale_type, "quantity": x.quantity,
                                                      "model": x.model if x.model != "none" else "",
                                                      "brand": x.brand if x.brand != "none" else "",
                                                      "serial": str(x.serial).split(".")[
                                                          0] if x.serial != "none" else ""})
...
for i in insertedData:
    ...
    for j, scale in enumerate(i["scales"]):
        ...
        fee = calc_fee(scale_type=scale['scale_type'], capacity=scale['capacity'])*total_quantity
        subtotal += fee
...
<td style="width: 
    ...
    driver.find_element(By.ID, "verification_date").send_keys("13-06-2024")
